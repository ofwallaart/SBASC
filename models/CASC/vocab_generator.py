...
class VocabGenerator:
    ...
    def __init__(self, cfg, save_results=True):
        self.cfg = cfg
    ...
    def __call__(self):
        aspect_categories = self.cfg.domain.aspect_category_mapper
        ...
        sentiment_categories = self.cfg.domain.sentiment_category_mapper
            ...
            with open(f'{self.root_path}/train.txt', encoding="utf8") as f:
                for line in tqdm(f):
                    ...
                    if not set(seeds[category]).isdisjoint(text.split()):
                        ...
                        word_scores, word_ids = torch.topk(word_predictions, self.cfg.model.K_1, -1)
